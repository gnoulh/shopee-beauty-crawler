...
with st.expander("Mục tiêu SMART (MT3)", expanded=False):
    st.markdown("""
...
def run_kmeans5(df, n = 5):
    ...
    km5  = KMeans(n_clusters=n, random_state=42, n_init=10)
...
st.subheader("Biểu đồ Elbow Method — Inertia theo k")
st.markdown("Biểu đồ dưới đây đo lường mức độ hội tụ (Inertia) của dữ liệu. Điểm gãy (Elbow) cho thấy k=5 là số lượng nhóm tối ưu để cân bằng giữa độ chính xác và tính phức tạp của mô hình K-Means trên log1p(price)×log1p(sold).")
...
# === Biểu đồ 1: Scatter ===
st.subheader("Biểu đồ 1: Bản đồ phân khúc — Giá vs Lượng bán (log scale)")
...
fig_s = px.scatter(samp, x="price", y="sold", color="cluster_name",
                   ...
                   custom_data=["name"] if "name" in samp.columns else None)
fig_s.update_traces(
    hoverinfo = "skip",
    hovertemplate=(
        # "<b>%{customdata[0]}</b><br><br>"
        "<b>Giá bán:</b> %{x:,.0f} VND<br>"
        "<b>Lượng bán:</b> %{y:,.0f}"
        "<extra></extra>"
    )
)
...
st.caption("Sample 3,000 sản phẩm. Log scale giúp thấy rõ phân bố khi giá thành (price) và lượt bán (sold) trải rất rộng.")

# ==================
# Biểu đồ 2: Revenue share
# ==================
st.markdown("---")
st.subheader("Biểu đồ 2: Tỷ trọng Doanh thu & Đặc trưng từng Phân khúc")

sc2 = cs.sort_values("rev_B", ascending=True).reset_index()

total_rev = sc2["rev_B"].sum()

fb = go.Figure()
for _, row in sc2.iterrows():
    pct = (row["rev_B"] / total_rev) * 100
    label_text = f"{row['rev_B']:.0f}B ({pct:.1f}%)"
    
    fb.add_bar(x=[row["rev_B"]], y=[row["tên"]], orientation="h",
               marker_color=cmap.get(row["tên"], CB_GRAY),
               text=[label_text], textposition="outside",
               name=row["tên"], showlegend=False, customdata=[pct])
fb.update_traces(
    hovertemplate = (
        "<b>Doanh thu:</b> %{x}tỷ VND <br>"
        "<b>Tỷ trọng:</b> %{customdata:.4f}%"
        "<extra></extra>"
    )
)
fb.update_layout(plot_bgcolor="white",
                 title="Doanh thu và Tỷ trọng theo phân khúc",
...
disp.columns = ["Tên phân khúc","Số SP","Giá TB (VND)","Lượt bán TB","DT (Tỷ đồng)","%DT"]
disp["Giá TB (VND)"] = disp["Giá TB (VND)"].apply(lambda x:f"{x:,.0f}")
disp["Lượt bán TB"] = disp["Lượt bán TB"].apply(lambda x:f"{x:,.0f}")
