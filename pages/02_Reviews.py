...
from utils.helpers import (
    ...
    extract_free, tokenize, get_top_keywords
)
...
# ============
st.subheader("Biểu đồ 1: Phân bố Rating & Độ dài Review")
# 1. Bộ lọc chọn 5 sao/ bỏ chọn 5 sao
filter_options = st.radio(":material/filter_alt: **Tùy chọn hiển thị dữ liệu:**", ("Tất cả", "Chỉ 1–3 sao", "Bỏ chọn 5 sao"), horizontal=True)
c1, c2 = st.columns(2)

# 1. Bộ lọc chọn 5 sao/ bỏ chọn 5 sao
# filter_options = st.radio("Lọc đánh giá:", ("Tất cả", "Chỉ 1–3 sao"), horizontal=True)

# 2. Lọc DataFrame dựa trên lựa chọn
if filter_options == "Chỉ 1–3 sao":
    plot_df = rv[rv["rating"] <= 3]
elif filter_options == "Bỏ chọn 5 sao":
    plot_df = rv[rv["rating"] < 5]
else: 
    plot_df = rv.copy()

# Khai báo Dictionary màu để cố định màu sắc cho từng mức sao, không bị lệch khi lọc
# Thang màu phân kì chuẩn (Mã màu HEX)
color_map = {
    1: "#E6550D",  # Cam đậm (Mức độ tiêu cực mạnh)
    2: "#FDAE6B",  # Cam nhạt (Mức độ tiêu cực vừa)
    3: "#FEE6CE",  # Cam/Be cực nhạt (Đóng vai trò trung tính/chuyển tiếp)
    4: "#9ECAE1",  # Xanh dương nhạt (Mức độ tích cực vừa)
    5: "#3182BD"   # Xanh dương đậm (Mức độ tích cực mạnh)
}
with c1:
    rc = plot_df["rating"].value_counts().sort_index()
    max_cnt = rc.max() if not rc.empty else 10
    # bar_c = [CB_VERMIL, CB_VERMIL, CB_ORANGE, CB_SKYBLUE, CB_BLUE]
    fig = go.Figure()
    for star, cnt in rc.items():
        pct = (cnt / len(plot_df)) * 100
        fig.add_bar(x=[star], 
                    y=[cnt], 
                    marker_color=color_map.get(star, "gray"),
                    # text=[f"{cnt:,}<br>({cnt/len(plot_df)*100:.1f}%)"],
                    text=[f"{pct:.1f}%"], # chỉ hiển thị phần trăm
                    textposition="outside",
                    cliponaxis=False, # đảm bảo text không bị cắt khi cao hơn trục y
                    name=f"{star} sao")
    fig.update_layout(showlegend = True, 
                      plot_bgcolor="white",
                      xaxis_title="Số sao", 
                      yaxis_title="Số đánh giá",
                      yaxis=dict(range=[0, max_cnt * 1.15]), # Mở rộng trục Y thêm 15% để lấy không gian chứa text
                      title=f"Phân bố rating – {len(plot_df):,} đánh giá"
    )
    st.plotly_chart(fig, use_container_width=True)

with c2:
    fig = go.Figure()
    for star in sorted(plot_df["rating"].unique()):
        d = plot_df[plot_df["rating"]==star]["review_length"].dropna()
        if len(d):
    ...
    fig.update_layout(plot_bgcolor="white", 
                      xaxis_title="Số sao",
                      yaxis_title="Độ dài review (ký tự)",
...
st.info(f"""
        **Nhận xét:**
        - {n_pos:,}/{len(rv):,} ({n_pos/len(rv)*100:.1f}%) là 5 sao — **J-curve điển hình TMĐT**.
        - Chỉ {n_neg} review 1–3 sao -> Dataset cực kỳ mất cân bằng, cần cân bằng trước khi dùng ML.
        - Review tiêu cực thường ngắn hơn tích cực — người thất vọng thường ít giải thích dài dòng.
        """)
...
c3, c4 = st.columns(2)
with c3:
    df_pos = get_top_keywords(pos_reviews["free_text"], top_n = 10)
...
with c4:
    df_neg = get_top_keywords(neg_reviews["free_text"], top_n = 10)
    fig = px.bar(df_neg, x="count", y="word", orientation="h",
...
st.info(f"""
**Nhận xét:**
* **5 sao nổi bật:** "Giao hàng nhanh" (vận chuyển), "Mùi thơm" (hương liệu), "Giá rẻ" (giá thành), "Cẩn thận" (đóng gói).
* **1–3 sao:** "Giá tiền", "Giống hình", "Giao hàng", "Hợp" -> Sản phẩm thường bị đánh giá là không đúng mô tả hoặc không xứng đáng với số tiền bỏ ra.
""")
# ============
# Biểu đồ 3: Phân loại 4 nhóm nguyên nhân
# ============
st.markdown("---")
st.subheader("Biểu đồ 3: Phân loại Nguyên nhân Đánh giá")

keywords_dict = {
    "Sự cố sản phẩm": ["dị ứng","ngứa","rát","mùi", "màu", "hương", "fake","giả","hỏng","chất lượng","lỏng","đặc", "khác mô tả"],
    ...
    "Vận chuyển": ["chậm","lâu","nhầm","sai","thiếu","shipper","giao hàng", "nhanh"],
}
...
def compute_categories(reviews_df):
    ...
    for cat, kws in keywords_dict.items():
        pattern = '|'.join(kws)
        has_kw = bad["review_text"].str.contains(pattern, regex=True, case=False)
        cat_counts[cat] = has_kw.sum()
...
if "has_image" in rv.columns:
    ...
    st.info(f"""
    **3 đặc trưng phân biệt 1–3 sao vs 5 sao:**

    1. **Tỷ lệ kèm ảnh:** Các đánh giá 1-3 sao có tỷ lệ kèm ảnh cao hơn ({ng:.1f}%) so với 5 sao ({ni:.1f}%) — khách hàng thường chụp ảnh để minh họa vấn đề khi không hài lòng.
    2. **Độ dài text tự do:** Trung bình {rv[rv['rating']<=3]['free_text_len'].mean():.0f} ký tự (1-3 sao) vs {rv[rv['rating']==5]['free_text_len'].mean():.0f} ký tự (5 sao).
    3. **Nội dung từ khóa:** "Giao hàng" luôn đứng đầu nguyên nhân tiêu cực; nhóm 1-3 sao tập trung vào "màu/hình", trong khi nhóm 5 sao tập trung vào "nhanh/thơm".
    """)
# # ============
